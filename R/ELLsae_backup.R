...
#'     
#'     n_boot_sub <- floor(n_boot / no_cores)
#'     rest <- n_boot %% no_cores
#'     if(rest == 0){rest <- no_cores}
#'     vector_of_num_boots <- c(rep(n_boot_sub, no_cores - 1), rest)
#' 
#'     y_bootstrap <- foreach(i=1:no_cores, 
#'                            #.packages=c("ELLsae"), 
#'                            .combine = "cbind") %dopar% {
#'                              ELLsae:::inferenceCensusC(n_bootstrap = vector_of_num_boots[i], 
