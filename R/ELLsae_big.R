...
ellsae_big <- function(model,
  ...
  if (!requireNamespace("bigstatsr", quietly = TRUE)) {
    stop(
      "Package \"bigstatsr\" needed for this function to work.
      Please install it, e.g. run install.packages(bigstatsr)",
      call. = FALSE
    )
  }
  ...
  if (missing(surveydata))
    stop("Data frame with the surveydata is missing")
  ...
  if (class(censusdata) != "data.table") {
    # alternativ if(!is.data.table(censusdata))?
  ...
  if (!(length(location_survey) == 1 &
    ...
    stop(
      "you have to provide a string with the variable indicating the location in the survey data set"
    )
  }
  ...
  if (any(is.na(surveydata[, ..location_survey]))) {
    warning(
      "There are missing values in the locations of your surveydata set. Rows with missing values were omitted"
    )
  ...
  if (!missing(mResponse)) {
    ...
    if (missing(location_census)) {
      if (any(location_survey == names(censusdata))) {
        ...
        stop(
          "if you want to use mResponse, you also have to provide a string indicating the name of the location variable in the census dataset.
    ...
    if (any(is.na(censusdata[, location_census]))) {
      stop(
        "The locations in the census are not allowed to have missing values if location means are supposed to be computed."
      )
    }
    ...
    if (mResponse == ".") {
      ...
      vars_for_mean_calculation <-
        gsub(pattern = " ", replacement = "" , mResponse)
      vars_for_mean_calculation <-
        unlist(strsplit(vars_for_mean_calculation, split = "\\+"))
      vars_for_mean_calculation <-
        unlist(strsplit(vars_for_mean_calculation, split = ","))
    } else if (is.character(mResponse)) {
      ...
      stop(
        "In order to include the means of variables included in the census in the model fit on the surveydata, you have to give a
    ...
    if (!all(mResponse %in%  names(censusdata))) {
      stop(
        "your input for mResponse includes variables that are not present in the censusdata set.
        Means for those variables cannot be calculated"
      )
    }
    if (!all(mResponse %in%  names(surveydata))) {
      warning(
        "your input for mResponse includes variables that are not present in the surveydata set.
    ...
    means_from_census <-
      unique(censusdata[, c(..new_var_names, ..location_census)])
    ...
    names(means_from_census)[names(means_from_census) == location_census] <-
      location_survey
    surveydata <-
      merge(
        surveydata,
    ...
    model_right_hand_side <- paste(model.in.characters[3],
                                   ...
                                   sep = " + ")
    model <-
      as.formula(paste(model_left_hand_side, model_right_hand_side, sep = " ~ "))
  }
  if (!missing(transf)) {
    y <- all.vars(model)[1]
    suveydata[, c(y) := transf(..y)]
  ...
  if (any(is.na(X_census))) {
    warning(
      "some explanatory variables in the census data set were missing. Affected rows were removed"
    )
  ...
  if (output == "default" |
      output == "all" | "bootsample" %in% output) {
  ...
  if (save_boot == T) {
    ...
    bigstatsr::big_write(
      bootstrap,
      paste("BootstrapSampleELLsae-", Sys.Date(),  ".csv", sep = ""),
